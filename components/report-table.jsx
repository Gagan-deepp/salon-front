...
export function CustomerMetricsTable({ data, loading = false }) {
  if (loading) {
    return (
          ...
          {rows.map((row, rowIndex) => {
            ...
            return (
              <TableRow
                key={row.key}
                className={`${
                  rowIndex % 2 === 0 ? "bg-muted" : "bg-gray-300"
                } hover:bg-slate-200 transition-colors`}
              >
                <TableCell
                  className={`sticky left-0 font-medium border-l border-primary-foreground text-slate-900 p-4 shadow-xl hover:bg-slate-200 transition-colors ${rowIndex % 2 === 0 ? "bg-muted" : "bg-gray-300"} `}
                >
                  {row.label}
                </TableCell>
                {metrics.map((item) => (
                ...
                <TableCell
                  className={`text-center p-4 font-medium border-l border-slate-200 tabular-nums ${growthColor(yoy)}`}
                >
                  {formatGrowth(yoy)}
                </TableCell>
                <TableCell
                  className={`text-center font-medium tabular-nums ${growthColor(mytd)}`}
                >
                ...
                <TableCell
                  className={`text-center font-medium tabular-nums ${growthColor(qoq)}`}
                >
                  {formatGrowth(qoq)}
                </TableCell>
              </TableRow>
          ...
          <Table className="w-full">
            ...
            <TableBody>
              {rows.map((row, rowIndex) => (
                ...
                >
                  <TableCell
                    className={`sticky left-0 font-medium text-slate-800 p-4 ${rowIndex % 2 === 0 ? "bg-white" : "bg-slate-50"}`}
                  >
                    {row.label}
                  </TableCell>
                  {metrics.map((item, idx) => (
...
export function ConsumableUsageTable({ data, loading }) {
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12">
        <div className="flex flex-col items-center gap-3">
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center p-12">
        <p className="text-muted-foreground">No consumable usage data available</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full overflow-x-auto rounded-lg border border-border">
      <Table className="w-full">
        <TableHeader>
          <TableRow className="bg-primary hover:bg-primary/90">
            <TableHead className="text-white font-medium p-4">Service Name</TableHead>
            <TableHead className="text-white font-medium p-4">Item Name</TableHead>
            <TableHead className="text-white font-medium p-4 text-center">Unit</TableHead>
            <TableHead className="text-white font-medium p-4 text-right">Qty Used</TableHead>
            <TableHead className="text-white font-medium p-4 text-right">Avg Cost/Unit</TableHead>
            <TableHead className="text-white font-medium p-4 text-right">Total Cost</TableHead>
            <TableHead className="text-white font-medium p-4 text-center">Usage Count</TableHead>
            <TableHead className="text-white font-medium p-4 text-right">Total Revenue</TableHead>
            <TableHead className="text-white font-medium p-4 text-right">Profit Margin</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {data.map((item, index) => (
            <TableRow
              key={`${item.serviceName}-${item.itemName}-${index}`}
              className={`${
                index % 2 === 0 ? 'bg-muted' : 'bg-gray-300'
              } hover:bg-slate-200 transition-colors`}
            >
              <TableCell className="font-medium p-4">{item.serviceName}</TableCell>
              <TableCell className="p-4">{item.itemName}</TableCell>
              <TableCell className="p-4 text-center">{item.unit}</TableCell>
              ...
              <TableCell className="p-4 text-right tabular-nums font-semibold">
                {formatCurrency(item.totalCost)}
              </TableCell>
              <TableCell className="p-4 text-center">
                {item.usageCount}
              </TableCell>
              <TableCell className="p-4 text-right tabular-nums font-semibold text-blue-600">
                {formatCurrency(item.totalRevenue)}
              </TableCell>
              <TableCell className="p-4 text-right tabular-nums font-semibold text-green-600">
...
export function ServiceProfitabilityTable({ data, loading }) {
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12">
        <div className="flex flex-col items-center gap-3">
